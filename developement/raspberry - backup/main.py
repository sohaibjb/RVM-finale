...
class MainWindow(QMainWindow):
    ...
    def listenToTriggers(self):
        ...
        widgets.butt_finish.clicked.connect(self.finish)
        # ///////////////////////////////////////////////////////////////
    ...
    #     #     self.exitApp()
    #
    #     # Set Finish and Help buttons
    #     if btnName == "butt_finish":
    #         # finish event
    #         self.finish()
    #     elif btnName == "butt_help":
    ##         # get a message to contact us
    ##         self.help()

    def finish(self):
        helpdial = QMessageBox(MainWindow)
        helpdial.setText("finish")
        helpdial.setStyleSheet("QLabel {min-width: 200px; min-height: 100px; }")
        helpdial.setInformativeText("thank you for your time")
    ...
    def clock(self):
        # dt = date.today()
        dt = datetime.now()
        widgets.time.setText(dt.strftime("%d-%m-%Y %H:%M:%S"))
    ...
    def save_image(self, img, user_class=""):
        class_name = "admin" + self.getClass(
            self.getClassfromRadioButton()) if self.page == "admin_page" else user_class
    ...
    def showClassOnRadioButton(self, class_id):
        ...
        if class_id == 1:
            widgets.radioButton_cam_glass1.setStyleSheet(
                "QRadioButton::indicator {border:6px solid rgb(0, 255, 0);background: rgb(255, 0, 0);}")
        elif class_id == 0:
            widgets.radioButton_cam_metal1.setStyleSheet(
                "QRadioButton::indicator {border:6px solid rgb(0, 255, 0);background: rgb(255, 0, 0);}")
        elif class_id == 3:
